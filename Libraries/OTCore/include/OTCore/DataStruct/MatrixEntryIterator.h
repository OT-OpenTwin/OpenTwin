...
namespace ot {
	...
	public:
		...
		MatrixEntryIterator(MatrixType* _matrix, const MatrixEntryPointer& _start, const MatrixEntryPointer& _end, bool _horizontalHeader = true);
		...
		inline MatrixEntryPointerStep getColumnStep() const { return (m_horizontalHeader ? MatrixEntryPointerStep::horizontal() : MatrixEntryPointerStep::vertical()); };
		inline MatrixEntryPointerStep getRowStep() const { return (m_horizontalHeader ? MatrixEntryPointerStep::vertical() : MatrixEntryPointerStep::horizontal()); };

		uint32_t getDataColumnIndex() const { return (m_horizontalHeader ? m_pointer.getColumn() : m_pointer.getRow()); };
		uint32_t getDataRowIndex() const { return (m_horizontalHeader ? m_pointer.getRow() : m_pointer.getColumn()); };
