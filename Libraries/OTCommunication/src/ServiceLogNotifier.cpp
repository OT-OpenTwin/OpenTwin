...
void ot::ServiceLogNotifier::setLoggingServiceURL(const std::string& _url) {
	if (_url.empty() || _url.find("//") == 0) {
		...
		std::string response;
		if (ot::msg::send("", _url, ot::EXECUTE_ONE_WAY_TLS, pingDoc.toJson(), response, 0, false, false)) {
...
void ot::ServiceLogNotifier::log(const LogMessage& _message) {	
	...
	try {
		...
		if (!ot::msg::send("", m_loggingServiceURL, ot::EXECUTE_ONE_WAY_TLS, doc.toJson(), response, 0, false, false)) {
