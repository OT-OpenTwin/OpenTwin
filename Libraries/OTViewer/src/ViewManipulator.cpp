 ...
 *
 * ViewManipulator code Copyright (C) 2010 PCJohn (Jan Peciva)
...
// OpenTwin header
#include "OTViewer/ViewManipulator.h"
...
/// Constructor.
ViewManipulator::ViewManipulator(int flags)
	: inherited(flags),
	_distance(1.),
	_trackballSize(0.8),
...
/// Constructor.
ViewManipulator::ViewManipulator(const ViewManipulator& om, const CopyOp& copyOp)
...
// doc in parent
void ViewManipulator::setTransformation(const osg::Vec3d& eye, const osg::Quat& rotation)
{
...
// doc in parent
void ViewManipulator::getTransformation(osg::Vec3d& eye, osg::Quat& rotation) const
{
...
// doc in parent
void ViewManipulator::setTransformation(const osg::Vec3d& eye, const osg::Vec3d& center, const osg::Vec3d& up)
{
	Vec3d lv(center - eye);
...
// doc in parent
void ViewManipulator::getTransformation(osg::Vec3d& eye, osg::Vec3d& center, osg::Vec3d& up) const
{
	center = _center;
	eye = _center + _rotation * osg::Vec3d(0., 0., _distance);
...
// doc in parent
bool ViewManipulator::handleMouseWheel(const GUIEventAdapter& ea, GUIActionAdapter& us)
{
	if (!handleMouseMovement) return false;
...
// doc in parent
bool ViewManipulator::performMovementLeftMouseButton(const double eventTimeDelta, const double dx, const double dy)
{
	if (!handleMouseMovement) return true;

	// rotate camera
	if (getVerticalAxisFixed())
		rotateWithFixedVertical(2 * dx, 2 * dy);
	else
		rotateTrackball(_ga_t0->getXnormalized(), _ga_t0->getYnormalized(),
			_ga_t1->getXnormalized(), _ga_t1->getYnormalized(),
			getThrowScale(eventTimeDelta) * 2);
	return true;
}


// doc in parent
bool ViewManipulator::performMovementMiddleMouseButton(const double eventTimeDelta, const double dx, const double dy)
{
	// pan model
...
// doc in parent
bool ViewManipulator::performMovementRightMouseButton(const double eventTimeDelta, const double /*dx*/, const double dy)
{
	// zoom model
...
bool ViewManipulator::performMouseDeltaMovement(const float dx, const float dy)
{
	// rotate camera
	if (getVerticalAxisFixed())
...
void ViewManipulator::applyAnimationStep(const double currentProgress, const double prevProgress)
{
...
bool ViewManipulator::startAnimationByMousePointerIntersection(
...
void ViewManipulator::OrbitAnimationData::start(const osg::Vec3d& movement, const double startTime)
{
...
void ViewManipulator::rotateTrackball(const float px0, const float py0,
...
void ViewManipulator::rotateWithFixedVertical(const float dx, const float dy)
{
	CoordinateFrame coordinateFrame = getCoordinateFrame(_center);
	Vec3d localUp = getUpVector(coordinateFrame);
...
void ViewManipulator::rotateWithFixedVertical(const float dx, const float dy, const Vec3f& up)
{
...
void ViewManipulator::panModel(const float dx, const float dy, const float dz)
{
...
/** Changes the distance of camera to the focal center.
	...
	\sa ViewManipulator::setMinimumDistance
 */
void ViewManipulator::zoomModel(const float dy, bool pushForwardIfNeeded)
{
	// scale
	float scale = 1.0f + dy;
 ...
 * (-1.0 ... 1.0)
 */
void ViewManipulator::trackball(osg::Vec3d& axis, float& angle, float p1x, float p1y, float p2x, float p2y)
{
	/*
 ...
 */
float ViewManipulator::tb_project_to_sphere(float r, float x, float y)
{
	float d, t, z;
...
void ViewManipulator::setCenter(const Vec3d& center)
{
	_center = center;
}
...
void ViewManipulator::setRotation(const Quat& rotation)
{
	_rotation = rotation;
}
...
void ViewManipulator::setMinimumDistance(const double& minimumDistance, bool relativeToModelSize)
{
...
/** Get the minimum distance of the eye point from the center
	before the center is pushed forward.*/
double ViewManipulator::getMinimumDistance(bool *relativeToModelSize) const
{
