...
#include "OTWidgets/Widgets/JsonTreeWidgetModel.h"
...
uint64_t ot::JsonTreeWidgetModel::countItems() const
{
	uint64_t count = 0;
	if (m_rootNode)
	{
		countItems(m_rootNode, count);
	}
	return count;
}
...
void ot::JsonTreeWidgetModel::countItems(const JsonTreeWidgetNode* _node, uint64_t& _count) const
{
	const auto& children = _node->getChildren();
	_count = _count + children.size();
