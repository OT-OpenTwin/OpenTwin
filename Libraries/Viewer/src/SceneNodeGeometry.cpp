...
ot::SelectionHandlingResult SceneNodeGeometry::setSelected(bool selected, const ot::SelectionData& _selectionData, bool _singleSelection, const std::list<SceneNodeBase*>& _selectedNodes, ot::VisualiserInfo& _visualisersInfo)
{
	ot::SelectionHandlingResult result = SceneNodeBase::setSelected(selected, _selectionData,_singleSelection, _selectedNodes, _visualisersInfo);
...
void SceneNodeGeometry::createOSGNodeFromEdges(double colorRGB[3], std::list<Geometry::Edge> &edges, osg::Node *&edgesNode, osg::Node *&edgesHighlightedNode, osg::Switch *&faceEdgesHighlightNode)
{
	...
	for (auto faceEdges : faceIdToEdgesList)
	{
		...
		osg::Node *faceEdgesNode = buildEdgesOSGNode(nEdges, vertices, 
													 ViewerSettings::instance()->geometryHighlightColor.r() / 255.0,
													 ViewerSettings::instance()->geometryHighlightColor.g() / 255.0,
													 ViewerSettings::instance()->geometryHighlightColor.b() / 255.0, 
...
{
	osg::Node* highlightNode = getEdgeHighlightNode(faceId1, faceId2, ViewerSettings::instance()->geometryHighlightLineWidth);
...
osg::Node* SceneNodeGeometry::getEdgeHighlightNode(unsigned long long faceId1, unsigned long long faceId2, double lineWidth)
{
	...
	osg::Node* edgeNode = buildEdgesOSGNode(nEdges, points,
											ViewerSettings::instance()->geometryHighlightColor.r() / 255.0,
											ViewerSettings::instance()->geometryHighlightColor.g() / 255.0,
											ViewerSettings::instance()->geometryHighlightColor.b() / 255.0, 
											1.0, false, lineWidth);
...
osg::Node* SceneNodeGeometry::addSelectedEdge(unsigned long long faceId1, unsigned long long faceId2)
{
	...
	osg::Node* selected = getEdgeHighlightNode(faceId1, faceId2, ViewerSettings::instance()->geometryHighlightLineWidth);
...
void SceneNodeGeometry::setHighlightColor(const ot::Color& colorValue)
{
	...
	osg::Geode* edgesHighlightedGeode = dynamic_cast<osg::Geode*>(edgeTransform->getChild(0));
	assert(edgesHighlightedGeode != nullptr);
