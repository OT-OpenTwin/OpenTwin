// @otlicense
// File: PropertyBundleVisVtkVectorVolumeTime.cpp
// 
// License:
...
#include "OTModelEntities/Properties/Bundle/PropertyBundleVisVtkVectorVolumeTime.h"

void PropertyBundleVisVtkVectorVolumeTime::setProperties(EntityBase * _thisObject)
{
	...
	EntityPropertiesSelection::createProperty(m_groupNameTime, m_properties.GetNameTime(), {}, "", m_defaultCategory, _thisObject->getProperties());
}

bool PropertyBundleVisVtkVectorVolumeTime::updatePropertyVisibility(EntityBase * _thisObject)
{
	bool changes = false;
	...
	EntityPropertiesSelection* time = dynamic_cast<EntityPropertiesSelection*>(_thisObject->getProperties().getProperty(m_properties.GetNameTime()));
...
bool PropertyBundleVisVtkVectorVolumeTime::is2dType(EntityBase* _thisObject)
{
	EntityPropertiesSelection* visType = dynamic_cast<EntityPropertiesSelection*>(_thisObject->getProperties().getProperty(m_properties.GetNameVisType()));
	assert(visType != nullptr);
...
void PropertyBundleVisVtkVectorVolumeTime::setTimeList(EntityBase* _thisObject, const std::list<double>& timeList)
{
	EntityPropertiesSelection* time = dynamic_cast<EntityPropertiesSelection*>(_thisObject->getProperties().getProperty(m_properties.GetNameTime()));
	...
	time->resetOptions(options);
	time->setValue(options.front());
}
