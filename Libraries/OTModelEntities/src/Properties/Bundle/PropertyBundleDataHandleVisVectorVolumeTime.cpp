// @otlicense
// File: PropertyBundleDataHandleVisVectorVolumeTime.cpp
// 
// License:
...
#include "OTModelEntities/Properties/Bundle/PropertyBundleDataHandle.h"
#include "OTModelEntities/Properties/Bundle/PropertyBundleDataHandleVisVectorVolumeTime.h"

PropertyBundleDataHandleVisVectorVolumeTime::PropertyBundleDataHandleVisVectorVolumeTime(EntityBase * thisObject)
{
	LoadCurrentData(thisObject);
}

void PropertyBundleDataHandleVisVectorVolumeTime::LoadCurrentData(EntityBase * thisObject)
{
	...
	auto timeEnt = dynamic_cast<EntityPropertiesSelection*>(thisObject->getProperties().getProperty(properties.GetNameTime()));
	assert(timeEnt != nullptr);

	if (timeEnt->getValue().empty())
	{
		time = 0.0;
	}
	else
	{
		time = std::stod(timeEnt->getValue());
	}
}
