...
ViewerUIDtype ViewerComponent::addMenuPushButton(ViewerUIDtype menuGroupID, const std::string &buttonName, const std::string &iconName, const ot::ContextMenu& _contextMenu)
{
	try {
		try {
			...
			ot::Flags<ot::ui::lockType> flags;
			flags.setFlag(ot::ui::tlAll);
			//flags.setFlag(ot::ui::tlViewWrite);
			flags.setFlag(ot::ui::tlViewRead);
...
void ViewerComponent::loadSettings(ot::SettingsData * _data) 
{
	...
	if (!_data->refreshValuesFromDatabase(AppBase::instance()->getDataBaseURL(), std::to_string(AppBase::instance()->getSiteID()), AppBase::instance()->getCredentialUserName(), AppBase::instance()->getCredentialUserPasswordClear(), AppBase::instance()->getCurrentUserCollection())) {
...
{
	if (!_data->saveToDatabase(AppBase::instance()->getDataBaseURL(), std::to_string(AppBase::instance()->getSiteID()), AppBase::instance()->getCredentialUserName(), AppBase::instance()->getCredentialUserPasswordClear(), AppBase::instance()->getCurrentUserCollection())) {
