...
IF %DEBUG%==1 (
	...
	"%DEVENV_ROOT_2022%\devenv.exe" "%SIM_PLAT_ROOT%\Libraries\KrigingService\KrigingService.vcxproj" %TYPE% "Debug|x64" /Out buildLog_Debug.txt
)

IF %RELEASE%==1 (
	ECHO %TYPE_NAME% RELEASE
	"%DEVENV_ROOT_2022%\devenv.exe" "%SIM_PLAT_ROOT%\Libraries\KrigingService\KrigingService.vcxproj" %TYPE% "Release|x64" /Out buildLog_Release.txt
) 
