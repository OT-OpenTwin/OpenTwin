...
extern "C"{
	...
	{
		const char *answer = ot::foundation::performAction(_json, _senderIP);
		return answer;
	};
	...
	{
		return ot::foundation::init(_localDirectoryServiceURL, _ownIP, _sessionServiceIP, _sessionID, &Application::instance(), false);
	};
}
