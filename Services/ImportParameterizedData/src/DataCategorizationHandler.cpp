...
void DataCategorizationHandler::AddSelectionsWithCategory(std::list<ot::UID>& selectedEntities, EntityParameterizedDataCategorization::DataCategorie category)
{
	...
	{
		assert(entityInfos.begin()->getEntityName() == _rmdPath);
...
void DataCategorizationHandler::StoreSelectionRanges(ot::UID tableEntityID, ot::UID tableEntityVersion, std::vector<ot::TableRange> ranges)
{
	...
	ot::EntityInformation entityInfo;
	_modelComponent->getEntityInformation(_scriptFolder, entityInfo);
	_scriptFolderUID = entityInfo.getEntityID();
	...
	{	
		for (size_t i = 0; i < ranges.size(); i++)
		{
			...
			if (allScripts.size() > 0)
			{
				...
				tableRange->createProperties(_scriptFolder, _scriptFolderUID, entityInfo.getEntityName(), entityInfo.getEntityID(), dataType);
			}
			else
			{
...
{
	std::string tableName = selectedPreviewTable.getEntityName();
	...
	auto baseEnt = _modelComponent->readEntityFromEntityIDandVersion(selectedPreviewTable.getEntityID(), selectedPreviewTable.getEntityVersion(), Application::instance()->getClassFactory());
	...
	{
		return std::make_pair<ot::UID, ot::UID>(selectedPreviewTable.getEntityID(),selectedPreviewTable.getEntityVersion());
	}
	else
	{
...
std::tuple<std::list<std::string>, std::list<std::string>> DataCategorizationHandler::CreateNewMSMDWithSelections(std::map<std::string, std::list<std::shared_ptr<EntityTableSelectedRanges>>>& allRelevantTableSelectionsByMSMD)
{
	...
	for (auto& elements : allRelevantTableSelectionsByMSMD)
	{
		...
		for (auto& selection : selections)
		{
			...
			newSelection->createProperties(_scriptFolder, _scriptFolderUID, selection->getScriptName(), entityInfo.getEntityID(),dataType,selectEntireRow,selectEntireColumn);
...
void DataCategorizationHandler::CheckEssentials()
{
	if (_rmdPath == "")
	{
		ot::EntityInformation entityInfo;
		std::list<std::string> allItems = _modelComponent->getListOfFolderItems(_baseFolder);
		_modelComponent->getEntityInformation(*allItems.begin(), entityInfo);
		_rmdPath = entityInfo.getEntityName();
	}
