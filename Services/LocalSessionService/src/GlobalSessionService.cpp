...
void GlobalSessionService::healthCheck(void) {
	...
	while (m_healthCheckRunning) {
		...
		// Send ping
		if (!ot::msg::send(m_owner->url(), m_serviceURL, ot::EXECUTE, pingMessage, response, ot::msg::defaultTimeout, ot::msg::DefaultFlagsNoExit)) {
		...
		// Check if last ping was not successful
			...
			response.clear();
			// Send registration
			if (!ot::msg::send(m_owner->url(), m_serviceURL, ot::EXECUTE, registerDoc.toJson(), response, ot::msg::defaultTimeout, ot::msg::DefaultFlagsNoExit)) {
