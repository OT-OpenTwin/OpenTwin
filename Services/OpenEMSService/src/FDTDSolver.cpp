...
void FDTDSolver::addPostprocessing(std::stringstream& runCommand)
{
	...
	runCommand <<
		...
		"    input_reference_impedance = np.asarray(\n"
		"        ports[input_port].Z_ref,\n"
		"        dtype=np.complex128\n"
		"    )\n"
		"\n"
		"    for output_port in sorted(ports.keys()):\n"
		"        output_reference_impedance = np.asarray(\n"
		"            ports[output_port].Z_ref,\n"
		"            dtype=np.complex128\n"
		"        )\n"
		"\n"
		"        input_resistance = np.real(\n"
		"            input_reference_impedance\n"
		"        )\n"
		"        output_resistance = np.real(\n"
		"            output_reference_impedance\n"
		"        )\n"
		"\n"
		"        if np.any(input_resistance <= 0.0):\n"
		"            raise ValueError(\n"
		"                f'Port {input_port} has a non-positive reference impedance.'\n"
		"            )\n"
		"\n"
		"        if np.any(output_resistance <= 0.0):\n"
		"            raise ValueError(\n"
		"                f'Port {output_port} has a non-positive reference impedance.'\n"
		"            )\n"
		"\n"
		"        impedance_normalization = np.sqrt(\n"
		"            input_resistance / output_resistance\n"
		"        )\n"
		"\n"
		"        s_parameter = (\n"
		"            ports[output_port].uf_ref\n"
		"            / incident_wave\n"
