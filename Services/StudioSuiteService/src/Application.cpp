...
std::string Application::processAction(const std::string & _action,  ot::JsonDocument& _doc)
{
	...
	else if (_action == OT_ACTION_CMD_UI_SS_HARNESS)
	{
		std::string harnessData = ot::json::getString(_doc, OT_ACTION_PARAM_FILE_Content);
		std::string harnessHash = ot::json::getString(_doc, OT_ACTION_PARAM_FILE_Hash);

		storeHarness(harnessData, harnessHash);
		return "";
	}
...
void Application::storeHarness(const std::string& harnessData, const std::string& harnessHash)
{
	std::string harnessEntityName = "Geometry/[Cable Harness]";

	infoFileManager.setHarnessHash(harnessHash);

	if (harnessHash.empty())
	{
		// Remove harness
		ot::ModelServiceAPI::deleteEntityFromModel(harnessEntityName, false);
		return;
	}

	if (harnessData.empty())
	{
		// The harness remains unchanged (the hash was the same, but harness data was present. The latter is indicated by a non-empy hash)
		return;
	}
	...
	// Now create the harness geometry entity	 
	int colorR = 150;
	int colorG = 150;
	int colorB = 240;
