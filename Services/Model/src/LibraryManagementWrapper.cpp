...
std::list<std::string> LibraryManagementWrapper::getCircuitModels() {
	...
	do {
		lmsResonse.clear();
		if (!(ok = ot::msg::send(Application::instance()->getServiceURL(), "127.0.0.1:8002", ot::EXECUTE, lmsDocs.toJson(), lmsResonse, ot::msg::defaultTimeout, ot::msg::DefaultFlagsNoExit))) {
