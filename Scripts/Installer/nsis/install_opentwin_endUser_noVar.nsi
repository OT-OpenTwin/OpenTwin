...
Function .onInit
	...
	#MessageBox MB_ICONINFORMATION|MB_OK "This installation will require a system restart at the end"
...
# DATABASE DIRECTORY OPERATIONS (ACTIVE)
	Function DatabaseEntry
		...
		${NSD_CreateText} 0 145 75% 12u "$INSTDIR\DataStorage\data"
			Pop $DirHandleDB
		...
		${NSD_CreateText} 0 200 75% 12u "$INSTDIR\DataStorage\log"
			Pop $DirHandleLog
...
Function RebootFunction
    MessageBox MB_YESNO "Are you sure you want to reboot now?" IDNO +2
    Reboot
FunctionEnd
; MUI Settings
	...
	#repurposing showreadme button to act as a reboot button
	!define MUI_FINISHPAGE_SHOWREADME
	!define MUI_FINISHPAGE_SHOWREADME_TEXT "Restart now"
	!define MUI_FINISHPAGE_SHOWREADME_FUNCTION RebootFunction
...
/*
	DetailPrint "Setting environment variables..."
			WriteRegStr HKLM "SYSTEM\CurrentControlSet\Control\Session Manager\Environment" "OPEN_TWIN_TOOLKIT_PORT" "8001"
				SendMessage ${HWND_BROADCAST} ${WM_SETTINGCHANGE} 0 "STR:Environment" /TIMEOUT=5000

			WriteRegStr HKLM "SYSTEM\CurrentControlSet\Control\Session Manager\Environment" "OPEN_TWIN_LOG_PORT" "$OPEN_TWIN_LOG_PORT"
				SendMessage ${HWND_BROADCAST} ${WM_SETTINGCHANGE} 0 "STR:Environment" /TIMEOUT=5000
			
			WriteRegStr HKLM "SYSTEM\CurrentControlSet\Control\Session Manager\Environment" "OPEN_TWIN_ADMIN_PORT" "8000"
				SendMessage ${HWND_BROADCAST} ${WM_SETTINGCHANGE} 0 "STR:Environment" /TIMEOUT=5000

			WriteRegStr HKLM "SYSTEM\CurrentControlSet\Control\Session Manager\Environment" "OPEN_TWIN_SITE_ID" "1"
				SendMessage ${HWND_BROADCAST} ${WM_SETTINGCHANGE} 0 "STR:Environment" /TIMEOUT=5000

			WriteRegStr HKLM "SYSTEM\CurrentControlSet\Control\Session Manager\Environment" "OPEN_TWIN_LOGGING_URL" "$OPEN_TWIN_SERVICES_ADDRESS:$OPEN_TWIN_LOG_PORT"
				SendMessage ${HWND_BROADCAST} ${WM_SETTINGCHANGE} 0 "STR:Environment" /TIMEOUT=5000

			WriteRegStr HKLM "SYSTEM\CurrentControlSet\Control\Session Manager\Environment" "OPEN_TWIN_LOGGING_MODE" "WARNING_LOG|ERROR_LOG"
				SendMessage ${HWND_BROADCAST} ${WM_SETTINGCHANGE} 0 "STR:Environment" /TIMEOUT=5000
	

	${If} $PublicIpSet <> 0 #public IP was set
			WriteRegStr HKLM "SYSTEM\CurrentControlSet\Control\Session Manager\Environment" "OPEN_TWIN_CA_CERT" "$PUBLIC_CERT_PATH\ca.pem"
				SendMessage ${HWND_BROADCAST} ${WM_SETTINGCHANGE} 0 "STR:Environment" /TIMEOUT=5000

			WriteRegStr HKLM "SYSTEM\CurrentControlSet\Control\Session Manager\Environment" "OPEN_TWIN_SERVER_CERT" "$PUBLIC_CERT_PATH\server.pem"
				SendMessage ${HWND_BROADCAST} ${WM_SETTINGCHANGE} 0 "STR:Environment" /TIMEOUT=5000

			WriteRegStr HKLM "SYSTEM\CurrentControlSet\Control\Session Manager\Environment" "OPEN_TWIN_SERVER_CERT_KEY" "$PUBLIC_CERT_PATH\server-key.pem"
				SendMessage ${HWND_BROADCAST} ${WM_SETTINGCHANGE} 0 "STR:Environment" /TIMEOUT=5000

	${Else} #public IP was NOT set
			WriteRegStr HKLM "SYSTEM\CurrentControlSet\Control\Session Manager\Environment" "OPEN_TWIN_CA_CERT" "$STANDARD_CERT_PATH\ca.pem"
				SendMessage ${HWND_BROADCAST} ${WM_SETTINGCHANGE} 0 "STR:Environment" /TIMEOUT=5000

			WriteRegStr HKLM "SYSTEM\CurrentControlSet\Control\Session Manager\Environment" "OPEN_TWIN_SERVER_CERT" "$STANDARD_CERT_PATH\server.pem"
				SendMessage ${HWND_BROADCAST} ${WM_SETTINGCHANGE} 0 "STR:Environment" /TIMEOUT=5000

			WriteRegStr HKLM "SYSTEM\CurrentControlSet\Control\Session Manager\Environment" "OPEN_TWIN_SERVER_CERT_KEY" "$STANDARD_CERT_PATH\server-key.pem"
				SendMessage ${HWND_BROADCAST} ${WM_SETTINGCHANGE} 0 "STR:Environment" /TIMEOUT=5000	
	${EndIf}						
*/
	; Shortcuts
