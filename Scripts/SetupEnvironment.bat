...
REM Single source of truth shared by the launcher scripts and the Visual Studio
REM debug launch configuration (.vs\launch.vs.json) generated by OTProject.cmake.
REM Calling the canonical script avoids duplicating these values here; every
REM variable inside it is guarded with "if not defined".
IF EXIST "%OPENTWIN_DEV_ROOT%\Scripts\Launcher\OpenTwin_set_up_services.bat" (
