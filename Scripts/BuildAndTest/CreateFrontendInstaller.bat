...
COPY %SYSTEM_32%\msvcp140_atomic_wait.dll		      		 %OPENTWIN_FRONTEND_DEPLOYMENT%
COPY %SYSTEM_32%\msvcp140_codecvt_ids.dll		      		 %OPENTWIN_FRONTEND_DEPLOYMENT%
...
COPY "%QT_DLLR%\QT6SvgWidgets.dll" 		"%OPENTWIN_FRONTEND_DEPLOYMENT%"
...
COPY "%QT_ADS_ROOT%\lib\qtadvanceddocking-qt6.dll" "%OPENTWIN_FRONTEND_DEPLOYMENT%"
...
REM OTCADEntities
COPY "%OT_CADMODELENTITIES_ROOT%\%OT_CDLLR%\OTCADEntities.dll" "%OPENTWIN_FRONTEND_DEPLOYMENT%"

REM OTDataStorage
COPY "%OT_DATASTORAGE_ROOT%\%OT_CDLLR%\OTDataStorage.dll" "%OPENTWIN_FRONTEND_DEPLOYMENT%"

REM OTServiceFoundation.dll
COPY "%OT_FOUNDATION_ROOT%\%OT_CDLLR%\OTServiceFoundation.dll" "%OPENTWIN_DEPLOYMENT_DIR%"

REM OTLTSpiceConnector
COPY "%OT_LTSPICE_CONNECTOR_ROOT%\%OT_CDLLR%\OTLTSpiceConnector.dll" "%OPENTWIN_FRONTEND_DEPLOYMENT%"

REM OTModelEntities
COPY "%OT_MODELENTITIES_ROOT%\%OT_CDLLR%\OTModelEntities.dll" "%OPENTWIN_FRONTEND_DEPLOYMENT%"

REM OTCommunication
COPY "%OT_COMMUNICATION_ROOT%\%OT_CDLLR%\OTCommunication.dll" "%OPENTWIN_FRONTEND_DEPLOYMENT%"

REM OTModelAPI
COPY "%OT_MODELAPI_ROOT%\%OT_CDLLR%\OTModelAPI.dll" "%OPENTWIN_FRONTEND_DEPLOYMENT%"

REM OTCore
COPY "%OT_CORE_ROOT%\%OT_CDLLR%\OTCore.dll" "%OPENTWIN_FRONTEND_DEPLOYMENT%"

REM OTGuiAPI
COPY "%OT_GUIAPI_ROOT%\%OT_CDLLR%\OTGuiAPI.dll" "%OPENTWIN_FRONTEND_DEPLOYMENT%"

REM OTGui
COPY "%OT_GUI_ROOT%\%OT_CDLLR%\OTGui.dll" "%OPENTWIN_FRONTEND_DEPLOYMENT%"

REM OTSystem
COPY "%OT_SYSTEM_ROOT%\%OT_CDLLR%\OTSystem.dll" "%OPENTWIN_FRONTEND_DEPLOYMENT%"

REM OTWidgets
COPY "%OT_WIDGETS_ROOT%\%OT_CDLLR%\OTWidgets.dll" "%OPENTWIN_FRONTEND_DEPLOYMENT%"

REM Rubberband
COPY "%OT_RUBBERBANDAPI_ROOT%\%OT_CDLLR%\OTRubberband.dll" "%OPENTWIN_FRONTEND_DEPLOYMENT%"
COPY "%OT_RUBBERBAND_OSG_ROOT%\%OT_CDLLR%\OTRubberbandOSG.dll"    "%OPENTWIN_FRONTEND_DEPLOYMENT%"

REM FrontendConnectorAPI
COPY "%OT_FRONTEND_CONNECTOR_API_ROOT%\%OT_CDLLR%\OTFrontendConnectorAPI.dll" "%OPENTWIN_FRONTEND_DEPLOYMENT%"

REM File Management Connector
COPY "%OT_FILE_MANAGER_CONNECTOR_ROOT%\%OT_CDLLR%\OTFMC.dll" "%OPENTWIN_FRONTEND_DEPLOYMENT%"

REM OTStudioSuiteConnector
COPY "%OT_STUDIO_SUITE_CONNECTOR_ROOT%\%OT_CDLLR%\OTStudioSuiteConnector.dll" "%OPENTWIN_FRONTEND_DEPLOYMENT%"

REM OTLTSpiceConnector
COPY "%OT_LTSPICE_CONNECTOR_ROOT%\%OT_CDLLR%\OTLTSpiceConnector.dll" "%OPENTWIN_FRONTEND_DEPLOYMENT%"

REM OTBlockEntities
COPY "%OT_BLOCKENTITIES_ROOT%\%OT_CDLLR%\OTBlockEntities.dll" "%OPENTWIN_FRONTEND_DEPLOYMENT%"

REM UI Core
COPY "%OT_UICORE_ROOT%\%OT_CDLLR%\uiCore.dll" "%OPENTWIN_FRONTEND_DEPLOYMENT%"

REM UI Service
COPY "%OT_UI_SERVICE_ROOT%\%OT_CDLLR%\uiFrontend.exe" "%OPENTWIN_FRONTEND_DEPLOYMENT%"
REN "%OPENTWIN_FRONTEND_DEPLOYMENT%\uiFrontend.exe" OpenTwin.exe

REM PythonExecution
COPY "%OT_PYTHON_EXECUTION_ROOT%\%OT_CDLLR%\PythonExecution.exe" "%OPENTWIN_FRONTEND_DEPLOYMENT%"

REM OTViewer
COPY "%OT_VIEWER_ROOT%\%OT_CDLLR%\OTViewer.dll" "%OPENTWIN_FRONTEND_DEPLOYMENT%"

REM ResultDatabaseAccess
COPY "%OT_RESULT_DATA_ACCESS_ROOT%\%OT_CDLLR%\OTResultDataAccess.dll" "%OPENTWIN_FRONTEND_DEPLOYMENT%"
