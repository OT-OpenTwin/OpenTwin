...
std::string AuthorisationService::generateToken2(const std::string& _token1)
{
...
void AuthorisationService::authorizeClient(const std::string& _token3)
{
...
void AuthorisationService::readUserName(CtxtHandle& _completedContext)
{
    ...
    if (QueryContextAttributesW(&_completedContext, SECPKG_ATTR_NAMES, &names) == SEC_E_OK)
    {
...
void AuthorisationService::readUserGroups(CtxtHandle _completedContext)
{
    HANDLE hUserToken = NULL;
    SECURITY_STATUS ss = QuerySecurityContextToken(&_completedContext, &hUserToken);
