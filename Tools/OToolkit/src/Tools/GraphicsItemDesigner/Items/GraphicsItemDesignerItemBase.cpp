...
void GraphicsItemDesignerItemBase::fillBasePropertyGrid(ot::PropertyGridCfg& _config) {
	...
	PropertyDouble* posXProp = new PropertyDouble("Position X", this->getGraphicsItem()->getGraphicsItemPos().getX(), 0., std::numeric_limits<double>::max());
	posXProp->setPropertyTip("Item top left X position.");
	generalGroup->addProperty(posXProp);
	PropertyDouble* posYProp = new PropertyDouble("Position Y", this->getGraphicsItem()->getGraphicsItemPos().getY(), 0., std::numeric_limits<double>::max());
